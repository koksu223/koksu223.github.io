                ...
                success: function (response) {
                    if (response === 'success') {
                        if (actionInput.length > 0) {
                            if (actionInput.val() === 'subscribe') {
                                $('.submit_subscribe').html('Subscribe Now <i class="rtmicon rtmicon-arrow-up-right"></i>'); 
                                const toast_comment = new bootstrap.Toast($('.success_msg_subscribe')[0]);
                                toast_comment.show();
                            } else if (actionInput.val() === 'appointment') {
                                form.find('.submit_appointment').html('Send Message <i class="rtmicon rtmicon-arrow-up-right"></i>');
                        ...
                        $('.submit_form').html('Send Message <i class="rtmicon rtmicon-arrow-up-right"></i>');
                        $('.submit_subscribe').html('Subscribe Now <i class="rtmicon rtmicon-arrow-up-right"></i>');
                        $('.submit_appointment').html('Send Message <i class="rtmicon rtmicon-arrow-up-right"></i>');
                    }
                }
            });
        }
